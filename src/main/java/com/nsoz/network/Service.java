...
public class Service extends AbsService {
    ...
    public void selectChar(Vector<Char> list) {
        try {
            ...
            for (Char _char : list) {
                ...
                ds.writeUTF(Char.setNameVip(_char.name)); // name
    ...
    public void openUIKhaiMoThucTinh( String title, String caption) {
        try {
            openUI((byte) 4, title, caption);
            Message ms = new Message(CMD.OPEN_UI_BOX);
            DataOutputStream ds = ms.writer();
            ds.writeInt(player.getCoinInBoxInt());
            Equip[] maskBox = player.equipment; //
            ds.writeByte(maskBox.length);
            for (Item item : maskBox) {
                if (item != null && item.template.type < 10) {
                    ds.writeShort(item.id);
    ...
    public void inviteWarClan(String name) {
        try {
    ...
    public void createClan() {
        try {
            ...
            DataOutputStream ds = ms.writer();
            ds.writeUTF(Char.setNameVip(player.clan.getName()));
            ds.writeInt(player.user.gold);
            ds.flush();
            sendMessage(ms);
            ms.cleanup();
    ...
    public void loadAll() {
        try {
            ...
            ds.writeByte(player.speed);
            ds.writeUTF(Char.setNameVip(player.name));
    ...
    public void viewInfo(Char pl) {
        try {
            ...
            DataOutputStream ds = ms.writer();
            ds.writeInt(pl.id);
            ds.writeUTF(Char.setNameVip(pl.name));
            ...
            if (pl != player) {
                updateInfoChar(pl);
                pl.serverMessage(Char.setNameVip(player.name) + " đang xem thông tin của bạn!");
            } else {
                updateInfoMe();
            }
        } catch (Exception ex) {
