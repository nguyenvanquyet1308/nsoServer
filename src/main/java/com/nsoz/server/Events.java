    ...
    public static void liXi(Char _char, String content) {
        ...
        if (NinjaUtils.nextBoolean()) {
            ...
            if (itemId == ItemName.BAT_BAO || itemId == ItemName.RUONG_BACH_NGAN || itemId == ItemName.RUONG_HUYEN_BI) {
                GlobalService.getInstance().chat("Hệ thống", Char.setNameVip(_char.name) + " lì xì nhận được " + itm.template.name);
            }
        ...
        receiver.addYen(yen);
        receiver.serverMessage("Bạn được " + Char.setNameVip(_char.name) + " lì xì " + NinjaUtils.getCurrency(yen) + " yên");
    }
