...
public class Server {
    ...
    public static void loadListVip() {
        try {
            ...
            stmt = conn.prepareStatement("SELECT * FROM `players` WHERE `tongnap` >= 1000000;");
            ResultSet resultSet = stmt.executeQuery();
            while (resultSet.next()) {
                String name = resultSet.getString("name");
                int tongnap = resultSet.getInt("tongnap");
                if (tongnap >= 1000000 && tongnap < 2000000) {
                    vip.add(name);
                } else if (tongnap >= 2000000 && tongnap < 5000000) {
                    vvip.add(name);
...
//            SpawnBossManager.getInstance().spawn(18, 0, 0, SpawnBossManager.THUONG, SpawnBossManager.ALL);
            SpawnBossManager.getInstance().spawnRepeat(SpawnBossManager.THUONG, 1, SpawnBossManager.RANDOM);
            SpawnBossManager.getInstance().spawn(6, 0, 0, SpawnBossManager.VUNG_DAT_MA_QUY, SpawnBossManager.RANDOM);
            SpawnBossManager.getInstance().spawn(9, 0, 0, SpawnBossManager.VUNG_DAT_MA_QUY, SpawnBossManager.RANDOM);
            SpawnBossManager.getInstance().spawn(21, 0, 0, SpawnBossManager.VUNG_DAT_MA_QUY, SpawnBossManager.ALL);
//            SpawnBossManager.getInstance().spawn(12, 0, 0, SpawnBossManager.LANG_TRUYEN_THUYET, SpawnBossManager.ALL);
//            SpawnBossManager.getInstance().spawn(22, 0, 0, SpawnBossManager.LANG_TRUYEN_THUYET, SpawnBossManager.ALL);
            SpawnBossManager.getInstance().spawnRepeat(SpawnBossManager.LANG_TRUYEN_THUYET, 4, SpawnBossManager.ALL);
            try {
                SpawnBossManager.getInstance().spawn(20, 0, 0, SpawnBossManager.HANG_VI_THU, SpawnBossManager.ALL);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            Clan.start();
